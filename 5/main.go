...
func runServer(ctx context.Context) error {
	...
	mux.HandleFunc("/hello", logRequest(handleHello)) // Добавляем middleware логирования
	mux.HandleFunc("/data", logRequest(handleData))   // Добавляем middleware логирования
...
func logRequest(next http.HandlerFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		log.Printf("%s %s, время выполнения: %v", r.Method, r.URL, time.Since(start))
	}
}
